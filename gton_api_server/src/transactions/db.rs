...
impl Transaction {
    pub async fn get_all(limit: i64, offset: i64, conn: &PgConnection) -> Result<Vec<TransactionSerde>> {
        let res = diesel::sql_query("SELECT tr1.tx_hash, tr1.stamp, tr1.amount_lp_in as amount, pr1.address FROM univ2_lp_remove_bsc_pancake AS tr1 
    ...
    pub async fn get_remove(
        ...
        let res = diesel::sql_query("SELECT t1.tx_hash, t1.stamp, t1.amount_lp_in as amount, p1.address FROM univ2_lp_remove_bsc_pancake AS t1 
        LEFT JOIN events_univ2_pair_created_bsc_pancake AS p1 on t1.pair_id = p1.id
...
// END AS NewField
    ...
    pub async fn get_swap(
        ...
        let res = diesel::sql_query("SELECT t1.tx_hash, t1.stamp, t1.amount_gton_out as amount, p1.address FROM univ2_buy_bsc_pancake AS t1 
        LEFT JOIN events_univ2_pair_created_bsc_pancake AS p1 on t1.pair_id = p1.id
         UNION SELECT t2.tx_hash, t2.stamp, t2.amount_gton_out as amount, p2.address FROM univ2_buy_eth_sushi AS t2
        LEFT JOIN events_univ2_pair_created_eth_sushi AS p2 on t2.pair_id = p2.id
        UNION SELECT t3.tx_hash, t3.stamp, t3.amount_gton_out as amount, p3.address FROM univ2_buy_ftm_spirit AS t3
